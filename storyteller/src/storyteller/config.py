...
class StoryTellerConfigDefaults:
    ...
    PAINTER_MODEL: str = "https://stablediffusionapi.com/api/v4/dreambooth"
    SPEAKER_MODEL: str = "tts_models/en/ljspeech/hifi-gan"
...
class StoryTellerConfigArgparseHelpText:
    ...
    _argparse_help_text: Dict[str, str] = {
        ...
        ): f"Text generation model to use. Default: 'gpt3'",
        _get_dataclass_var_name_from_f_string_eq(
            f"{StoryTellerConfig.painter=}"
        ): f"Image generation model to use. Default: 'dalle2'",
        _get_dataclass_var_name_from_f_string_eq(
